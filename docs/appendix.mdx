...
To configure processors, **Director** uses YAML files in which plurals are represented either as a comma-separated list of items within a pair of brackets&mdash;e.g.
		```code
			["A", "B", ...]
		```
or as a multi-line list with `-` indicating entries&mdash;e.g.
		```code
			- "A"
			- "B"
			- ...
...
**any** A value that can be represented by alphanumeric strings
...
> _plu_. **strings...** One or more quoted strings separated by space (e.g. &blank;) or a delimiter (e.g. `,`).

**script** A quoted string containing expressions that can be evaluated by an interpreter&mdash;e.g.
	```code
	_ingest.vendor.ip != null
	```
**shape** A shape description standard used for maps.
