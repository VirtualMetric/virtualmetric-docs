...
Fields can be referred to in other fields in two ways:
...
- Using the so called _mustache_ syntax for concatenation:
...
Apache Parquet is a column-oriented binary storage format designed for efficiency:
...
:::info
A third-party tool is required to access and query the contents of these files.
:::
...
Files have Base64-encoded content, have `BEGIN`/`END` markers, can contain multiple certificates/keys, and are text-based.
