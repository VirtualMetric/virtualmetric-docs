...
Cisco's event streaming protocol used by Firepower Management Center to send events to external clients.

### IPFIX

The IP Flow Information Export is a protocol for exporting IP flow information from routers, switches, and other network devices.
...
A network protocol developed by Cisco for collecting IP traffic information.

### sFlow

A network monitoring protocol for packet sampling in high-speed networks.
