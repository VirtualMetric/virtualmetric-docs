...
This opens a the multi-tab view used for creating the target.

On the top left, you will see the **Actions** dropdown which offers the option to

- **Clone Target** - Make a new copy of the target
- **Disable/Enable Target** - Selecting the currently visible options toggles the menu item
- **Delete Target** - Delete the target

On the left, you will see the settings made at creation or last update. To modify these settings, click the blue **Manage target details** button.

This activates the form for editin. After making your changes, click **Save changes** to update the target, or **Cancel** to discard your changes.
...
:::warning
Target deletion is permanent and immediately stops all data flows to the target's destination.
:::
