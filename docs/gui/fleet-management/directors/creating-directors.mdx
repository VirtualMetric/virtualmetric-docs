...
- Open a terminal window with _admin_ access
...
- the button is clicked without an installed director, or before the installation completes
- the local machine is in an invalid state
- the fireewall settings on the local machine prevents the connection

In the event of these, the process will return an error notification.
:::
