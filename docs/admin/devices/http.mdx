...
The minimum required settings for a basic server are:
...
For authentication, the minimum required settings use:
...
To enable API key authentication, use:

- a custom header-based authentication
- a configurable header name and value
- an environment variable for secure key storage
...
For a secure HMAC authentication, use:
...
For a full security configuration, the following are necessary:
