...
In order to tweak performance and data handling, the following settings are used.
...
### Basic UDP Server with Checkpoint Pre-processing Pipeline
...
:::note
The `checkpoint` is a pre-processing pipeline that extracts Checkpoint firewall logs from syslog messages.
If the device is a Checkpoint firewall, this pipeline will parse the logs and extract relevant fields for further processing.
If the device is not a Checkpoint firewall, the pipeline will have no effect on the incoming messages.
:::
