...
:::warning
Note, however, that as regular expressions can slow down indexing operations, it is better to avoid them whenever possible.
:::
...
:::note
Again, it is better to avoid using complex condition scripts as they may slow down the process.
:::
...
```js
ctx.network?.name.equalsIgnoreCase('Guest')
```
as
...
## Running Pipelines Conditionally
...
"processors": [
   {
      "pipeline": {
         "if": "ctx.service?.name == 'foo'",
         "name": "foo_pipeline"
      }
   },
   {
      "pipeline": {
         "if": "ctx.service?.name == 'bar'",
         "name": "bar_pipeline"
      }
   },
   {
      "fail": {
         "if": "ctx.service?.name != 'foo' && ctx.service?.name != 'bar'",
         "message": "It is required to have `foo` or `bar`"
      }
   }
]
```
