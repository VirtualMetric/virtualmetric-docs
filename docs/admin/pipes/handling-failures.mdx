...
To specify the processors to run immediately after a processor fails, the `on_failure` parameter can be used.
...
For cascading error, multiple `on_failure` processors can be nested.
