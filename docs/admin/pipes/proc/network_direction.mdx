...
|`destination_ip`|String|N|`destination.ip`|Field containing the destination address|
...
|`internal_networks`|Strings|N|`*`|List of internal networks. Supports IPv4 and IPv6 addresses and ranges in CIDR notation, as well as the named ranges listed below. Only `internal_networks` or `internal_networks_field` must be specified. If the latter, then follows the behavior of `ignore_missing`|
|`internal_networks_field`|String|N|-|The field containing the internal networks|
...
|`source_ip`|String|N|`source.ip`|Field containing the source address|
|`tag`|String|N|-|Identifier|
|`target_field`|String|N|`network.direction`|The field to assign the network direction|
