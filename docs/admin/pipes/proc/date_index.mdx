...
<Synopsis>
Points documents to the right time-based index according to the date or timestamp field in a document.
</Synopsis>
...
|`field`|Y|N/A|The field to get the date or timestamp from|
...
Points documents to a monthly index that starts with a `the_index-` prefix based on a date in the `the_date` field:
...
```js
{
   "the_date": "2024-06-55T09:05:11.3346"
}
