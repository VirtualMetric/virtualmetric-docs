...
<Synopsis>
Allows routing a document to another target index or data stream.
</Synopsis>
...
The processor first tries to resolve the `service.name` to determine the dataset. If that field contains `null`, does not exist, or is not a string, it tries the next element in the list. Here, this is the literal "`generic`". The `namespace` is configured with a single literal value:
