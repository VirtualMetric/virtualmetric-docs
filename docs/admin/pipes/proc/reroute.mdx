...
After a `reroute` processor has been executed, all the other processors in the current pipeline are skipped. If the current pipeline is executed in the context of a _Pipeline_, the calling pipeline will be skipped as well. This means that at most one `reroute` processor is ever executed within a pipeline, allowing to define mutually exclusive routing conditions.
