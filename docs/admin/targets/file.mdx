...
Creates a file target that writes log messages to files in various formats (JSON, Avro, OCF, Parquet) with support for different compression methods and schemas.
...
|Field|Required|Default|Description|
|---|---|---|---|
|`location`|N|Service root path|File output directory|
...
The following variables can be used in the file name template:
...
### Compressed Parquet with Schema
...
Configuration for Windows environment with proper path structure:
...
:::note
If no schema is specified for Avro, OCF, or Parquet formats, a default schema will be used that captures epoch timestamp and message content.
:::

:::warning
When `no_buffer` is enabled, each write operation will be immediately flushed to disk. This provides better durability but may impact performance.
:::

:::note
Files with no messages (counter = 0) are automatically removed when the target is disposed.
:::