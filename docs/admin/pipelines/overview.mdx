...
Pipelines are chains of processors that run sequentially, operating on the incoming data streamed from [sources](../sources/overview.mdx), and routing their output to [targets](../targets/overview.mdx) for consumption or further processing.
...
:::note
While reading the synopses, to understand the type specifiers, use the [Appendix](/docs/appendix.mdx#synopsis-type-specifiers).
:::
