...
Pipelines are chains of [processors](./processors) that run sequentially, operating on the incoming data streamed from [sources](source.mdx), and routing their output to [targets](target.mdx) for consumption or further processing.
