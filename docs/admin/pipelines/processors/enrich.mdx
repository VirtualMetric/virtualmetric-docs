...
This involves correlating data items to increase their semantic depth and relevance for the target. A typical example would be to grab the IP of a Website user, look up the IP range from a _geolocation_ database, and infer what _geographical region_ the user is using the site from.
