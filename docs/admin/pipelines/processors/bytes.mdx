...
:::note
Supported units are `B`, `KB`, `MB`, `GB`, `TB`, and `PB` (case-insensitive). Fractional values like "`1.5KB`" are supported. An error is raised if the value exceeds the 64-bit integer limit.
:::