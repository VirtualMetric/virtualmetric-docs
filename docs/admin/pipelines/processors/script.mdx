...
<Example>
   ...
   <Description>
   Extract the `tags` field from the `env` source field
   </Description>
   <SampleCode>
   ```yaml
   processors:
   ...
   <SampleCode>
   ```json
   {env: "foo", tags: ["foo-tags"]}
   ```
   </SampleCode>
</Example>

* Using an double-quoted **inline** script:
...
<Example>
   ...
   <SampleCode>
      ```json
      {dataset: "foo", locale: "en-UK"}
      ```
   </SampleCode>

   <Description>
      Use the _locale_ and the _dataset name_...
   </Description>
   <SampleCode>
   ```yaml
   processors:
