...
**Functionality**: Flattens nested objects into a single-level object where keys are paths representing the original nested structure. The original nested structure in the source field is preserved when `target_field` is used.
