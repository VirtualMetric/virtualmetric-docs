...
**Functionality**: Intentionally fails the pipeline with a custom error message. The message can include templates that reference document fields.
...
:::note
When `ignore_failure` is set to `true`, errors are still logged but they don't stop the processing.
:::
