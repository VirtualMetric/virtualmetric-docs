...
* locates the specified field in the document, and determines if the field contains an array or a map

* if it does, it goes through each element storing the current value in `_ingest._value`, the current key/index in `_ingest._key`, executing the specified processor, and capturing any errors for failure handling. 
