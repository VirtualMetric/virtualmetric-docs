...
As can be seen from the graphics, the components in the middle of this setup are actually called [routes](/docs/administration/routes/overview.mdx).
...
Additionally, their selection will generally be dictated by the consumers downstream. This is where premature optimization, as usual, can be a sources of frustration.
...
_Pre-processing_ pipelines are attached to _sources_, and prepare the data before it enters the routing stage. They focus on:
...
_Normalization_ pipelines handle the conversion between different log formats throughout the processing chain. The primary transformations are field name standardization, data type normalization, structure unification, and time format alignment.

_Post-processing_ pipelines are attached to _targets_, and perform final transformations before data storage and analysis:
...
Keep the pipelines focused on their _purpose_, and minimize cross-type dependencies. Leverage the seperation of concerns this brings; it

- clarifies the responsibility of each by assigning a specific role
- enables modular architecture and robust testing, and
- reduces overhead through better resource utilization, and improves scalability and routing efficiency.
