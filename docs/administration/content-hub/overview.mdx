...
These categories can be used to filter the content you see in the viewing area.

## Filtering

**Device type** is single select, **Target** and **Device vendor** are multi-select.
...
To clear a device type filter, click the `x` next to the selected item.

For multi-select filters (**Target** and **Device vendor**), uncheck items to remove them from the filter.
...
To edit a template, users must first install it.

After installation, the template content appears as a pipeline in **DataStream**. See <Topic id="administration-managing-templates-installation">**this section**</Topic> for installation and management steps.
