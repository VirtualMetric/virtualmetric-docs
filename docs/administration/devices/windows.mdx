...
|`port`|N|`???`|WinRM port number|
|`use_ad_credentials`|N|`true`|Use _Active Directory_ credentials|
|`username`|N|`<AD-usename>`|Must be specified if `use_ad_credentials` is `false`|
|`password`|N|`<AD-password>`|Must be specified if `use_ad_credentials` is `false`|
