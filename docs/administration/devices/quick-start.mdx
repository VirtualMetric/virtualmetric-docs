...
The user has the flexibility to organize the device configuration files as they wish: the files a `.yml` extension and reside inside the `config` directory:
...
Director searches the `<vm_root>/config` directory recursively for files with `.yml` extension. You can organize your configurations in several ways:
