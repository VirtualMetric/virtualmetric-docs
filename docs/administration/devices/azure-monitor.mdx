...
<ExampleGrid>
  <CommentCol>
    Create a basic Azure Monitor collector...
  </CommentCol>
  <CodeCol>
    ```yaml
    - id: 1
...
We can collect multiple log types with pre-processing:

<ExampleGrid>
  <CommentCol>
    Specify multiple log streams with custom processing...
  </CommentCol>
  <CodeCol>
    ```yaml
    - id: 2
...
We can enhance performance when dealing with high log volumes:
