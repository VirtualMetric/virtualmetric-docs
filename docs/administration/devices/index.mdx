...
**Director** offers flexible organization options for device configuration files: they all have a `.yml` or `.yaml` extension, and reside inside the `config` directory whose structure is like so:
