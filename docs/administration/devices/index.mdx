...
**Director** offers flexible organization options for device configuration files: they all have a `.yml` extension, and reside inside the `config` directory whose structure is like so:
