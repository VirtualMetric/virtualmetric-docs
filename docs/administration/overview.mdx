...
To help design the processes that will define, combine, and run the above stages, **Director** uses the files in the hierarchical folder structure under `vm_root`:
...
flowchart LR
   ...
   config@{shape: docs, label: "<code>config/</code>"}
      devices@{shape: tag-doc, label: "<code>devices</code>"}
         syslog("<code>syslog.yaml</code>")
         estreamer("<code>estreamer.yaml</code>")
         tcp("<code>tcp.yaml</code>")
      targets@{shape: tag-doc, label: "<code>targets</code>"}
         sentinel("<code>sentinel.yaml</code>")
         storage("<code>storage.yaml</code>")
      routes@{shape: tag-doc, label: "<code>routes</code>"}
         main("<code>main.yaml</code>")
      vmetric("<code>vmetric.yaml</code>")
   ...
   package@{shape: docs, label: "<code>package/</code>"}
      packageDefinitions@{shape: docs, label: "<code>definitions/</code>"}
         packageDefinitionsPipelines@{shape: tag-doc, label: "<code>pipelines/</code>"}
            packageDefinitionsPipelinesCheckpoint("<code>checkpoint.yaml</code>")
            packageDefinitionsPipelinesCisco("<code>cisco.yaml</code>")
            packageDefinitionsPipelinesNormalize("<code>normalize.yaml</code>")
   ...
   user@{shape: docs, label: "<code>user/</code>"}
      userDefinitions@{shape: docs, label: "<code>definitions/</code>"}
         userDefinitionsPipelines@{shape: tag-doc, label: "<code>pipelines/</code>"}
            userDefinitionsPipelinesCheckpoint("<code>checkpoint.yaml</code>")
            userDefinitionsPipelinesNormalize("<code>normalize.yaml</code>")
...
The configurations for each type of component have their own directory. The YAML files contain predefined fields these components recognize, and **Director** uses these as settings to spawn and run its processes.

### Definition Files

Two directories are of key importance: `package` and `user`.
...
System administration frequently involves dealing with failures. The most common issues are configuration errors, connection problems, unhandled processing errors, and violated resource constraints.

The resolution generally involves checking the configurations and reviewing the logs, verifying connectivity and resource availability, and testing the solutions.
