...
- **Filtering** - Use the filter next to the search box to narrow pipelines by device type.
...
To update the child pipeline, click the **Save changes on the pipeline** button, or, on the left sidebar, click the check mark next to the name of the child pipeline.
