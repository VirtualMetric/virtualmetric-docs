...
|`-filePath`|-|Path to file containing messages|
...
* With File Input:
  ...
  ```bash
  vmetric-generator -protocol=tcp -address=127.0.0.1:1514 -filePath=./test-messages.txt -count=100
  ```
...
* Basic Usage:
  ...
  ```bash
  vmetric-generator -protocol=vmf -filePath=./output.vmf
  ```

* With Custom Content:

  ```bash
  vmetric-generator -protocol=vmf -filePath=./metrics.vmf -message="Custom VMF data" -count=1000
  ```
...
* Using Message Files:
  ...
  ```bash
  vmetric-generator -protocol=tcp -address=127.0.0.1:1514 -filePath=./sample-logs.txt -count=100
  ```
