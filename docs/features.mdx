...
**VirtualMetric Director** offers many ground-breaking features to streamline your telemetry operations, making them easier to manage and more efficient.

## Vectorized Processing 

**VirtualMetric Director**'s vectorized pipeline engine is designed for maximum efficiency.

By utilizing all available cores, it processes large log volumes in record time and provides parallel data ingestion to target SIEMs.

With over **10x** the ingestion speed of traditional solutions and up to **99%** disk and network compression, **VirtualMetric Director** reduces bandwidth and disk usage for queuing down to the bare minimum&mdash;delivering unmatched performance and cost savings.
...
Unlike classic solutions that focus solely on data collection via protocols or third-party agents, _Datasets_ categorize data&mdash;Windows Event Logs, Linux Audit Logs, Windows User Activity&mdash;at the source, simplifying pipeline design and enabling advanced **RBAC**.

With _Datasets_, users can define role-based access controls at the data access level to ensure that teams working on the same source are fully isolated and unable to view each other's data. This innovative approach delivers greater flexibility and tighter security.

{/* TODO: Link */}

## Agentless Data Collection

**VirtualMetric Director**'s agentless design enables effortless data collection from Windows, Linux, Unix, MacOS, Solaris, AIX, and many more without requiring third-party tools or complicated configurations. 

Our system leverages read-only user rights for secure remote access, ensuring data integrity and compliance.

By integrating with _Credential Stores_ or _Active Directory Service Accounts_, **VirtualMetric Director** eliminates the need for user credentials, simplifying creation of secure connections.
...
## Zero Data Loss Pipeline Engine
...
{/*## Batching Support TODO:*/}

{/*## Disk Persistency TODO:*/}
