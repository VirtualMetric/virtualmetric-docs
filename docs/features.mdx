...
With the 50+ processors it provides, **VirtualMetric Director** has the most comprehensive support in the industry for _low-code_/_no-code_ management enabling tasks like parsing, filtering, enrichment, routing, and more. Engineers with **Elastic** experience can leverage this robust and flexible pipeline engine, reducing onboarding time and enhancing operational efficiency.

## Lossless Pipeline Engine

**VirtualMetric Director**'s _Write-Ahead Log_ (WAL) architecture provides a robust foundation for data integrity by securely storing all routing and pipeline states on disk. This ensures zero data loss, even in the event of a crash.

Unlike solutions that require additional components like Kafka, **VirtualMetric Director** caps log duplication at just one message.
...
With over _10 times_ the ingestion speed of traditional solutions and up to _99 percent_ disk and network compression, **VirtualMetric Director** reduces bandwidth and disk usage for queuing down to the bare minimum, delivering great cost savings.
