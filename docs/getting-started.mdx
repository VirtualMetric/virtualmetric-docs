...
Now you can right-click `vmetric-director.exe` and select **Run as Administrator** from the context menu, and it will run as a service.

## Run: Checkpoint Simulation

Once again, open a **PowerShell** terminal with _Admin_ rights and, if you haven't added it to `PATH` yet, cd to `<vm_root>`. Then type the executable name and press <kb-short>Enter</kb-short>.
...
{/* TODO: Buralarda Mark'ın notlarından bir şey anlamadım. Ne yapıyoruz tam olarak? */}
