...
Enterprise security teams need to optimize their Azure Sentinel deployment for cost-efficiency while maintaining comprehensive security monitoring. This tutorial implements advanced Sentinel integration that:
...
**Data flow:**
Raw Logs → ASIM Normalization → DCR Routing → Azure Sentinel Tables → Analytics Rules
...
- **Target**: Azure Sentinel with automated DCR management
...
**Output**: ASIM-normalized events ready for Azure Sentinel ingestion
...
### Step 2: Configure Azure Sentinel Target with DCR Automation

```yml title="azure-sentinel-advanced.yml"
targets:
  # Azure Sentinel with automated DCR creation
...
### Step 5: Start Azure Sentinel Integration
...
In Azure Sentinel, create analytics rules using the normalized data:
...
2. **Ingestion Costs**: Monitor Azure Sentinel data ingestion charges
...
:::caution
Azure Sentinel ingestion incurs costs based on data volume. Monitor usage carefully and implement appropriate cost controls for production deployments.
:::