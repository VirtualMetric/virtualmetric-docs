...
Follow the on-screen prompts to complete the setup process. For a detailed step-by-step instructions, refer to [Microsoft Sentinel Automation](./index.mdx).
...
### Data Collection Rules Permissions
...
**Microsoft Sentinel** has moved to DCR-based log ingestion and manual schema management. This change, while powerful, can lead to _phantom fields_, i.e. data fields that are ingested and billed even though they are not part of the table schema and therefore are not accessible for querying, and yet are still incurring storage costs.
...
**Director**'s autodiscovery feature includes a built-in phantom field prevention using the following:
...
The following should be observed as guiding principles:
