...
To download, install, and run **VirtualMetric** binaries **Director**, **Generator**, and **Agent**, follow the instructions in the relevanet sections below.
...
:::warning[attention]
**Director**  does _not_ require administrative privileges to run as a background process. However, if you choose to install it as a _service_, administrative privileges are required. Once installed, you can run it with regular user privileges.
:::
...
<Tabs>
  <TabItem value="powershell" label="PowerShell" default>
    ...
    cp C:\Users\<admin-user>\Downloads\vmetric-director.exe "C:\Program Files\VirtualMetric"
...
To simulate streaming data, **Director** provides a specific mode named `generator`. It can be used to send messages to specific logging services, although other traditional tools&mdash;such as `logger`&mdash;can also be used for this purpose on some **Unix**-based platforms.

{/* TODO: Write the generator check */}

<Link to="./introduction#deployment" className="button button--primary" style={{ width: '400px', display: 'inline-block', textAlign: 'center'}}>Back To Introduction</Link>
...
**Agent** is a single binary that requires no dependencies, 
