...
This chapter will help you get started with targets to write processed data to specific formats, walking you through some common scenarios.
...
:::note
Each target type provides specific options detailed in its respective [chapter](/docs/configuration/targets/).
:::
...
The most basic target we can direct our output is a _console_. For this, we have to create a simple `stdout` configuration:
...
The `id` field value (`1` in our example) is the unique identifier of the target. The type is  `console`, and we intend to normalize the data to the [ECS](/docs/appendix.mdx#ecs) format, although this is optional.
...
For this type of configuration, we have to specify an **Azure** account, which requires a client id and secret for security. The size we want for storage is roughly 512MB.
...
You can place this in a file name `config/targets/sentinel.yml`.

This configuration specifies the `sentinel` type. Once again, we have to specify our **Azure** account information. For this type of target, we also need to specify the type of stream we are using, and since that is ASIM, we have entered two names for our custom ASIM-based storage.
