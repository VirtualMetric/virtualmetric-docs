...
Put this configuration in a file named `console.yml`, and for convenience place it directly under `config`.
...
Put this in a file named `azblob.yml` and place it under `config`.
...
Put this in a file named `sentinel.yml` placed under `config`.
