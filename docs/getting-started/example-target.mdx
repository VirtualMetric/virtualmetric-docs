...
You can put this configuration in a file named `console.yml`, and for convenience place the file directly under `config`.
...
Place this in a file named `azblob.yml` under `config`.
...
Put this in a file named `sentinel.yml` under `config`.

This configuration uses the `sentinel` type. Once again, we have to specify our **Azure** account information. For this target, we also need to specify the type of stream we are using. Since that is ASIM, we have entered two names for our custom ASIM-based storage tables.
...
Finally, you can configure multiple targets for redundancy, implement normalization rules, and put alerts in place for notifications and error handling.
