...
By default, target configuration files reside in the `targets` directory under the `config` folder and have a `.yml` extension:
...
Your target file can be named as, e.g.
