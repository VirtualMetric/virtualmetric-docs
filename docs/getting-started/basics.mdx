...
:::tip
You can place your files anywhere you wish under `config`. **Director** discovers all of them by traversing the subdirectories recursively.
:::
...
You can invoke the processes you want to run using **VirtualMetric Director** using a terminal. Here are a few typical use cases:

* For _Syslog_ collection:
...
* For running a specific pipeline with validation:

  <Tabs groupId="os-options">
    <TabItem value="windows" label="Windows" default>
      ```powershell
      vmetric-director -mode=pipeline -name=pipeline_name -input=input_file.json -output=output_file.json -validate
      ```
    </TabItem>
...
* For viewing logs with a specific debug level:
...
This route receives its data from all devices with the prefix `syslog_` in their names, uses the `normalize` and `enrich` pipelines to extract and transform the received data points, and then forwards them to the target named `backup`.
