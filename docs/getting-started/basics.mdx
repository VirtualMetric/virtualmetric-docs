...
The configurations may be placed in separate files or they may be grouped together logically, i.e. based on how they are intended to be used or the type of data streams they process.
...
It is also possible to use _standalone_ files placed in any arbitrary directory you wish. In that case, the full path of the file must supplied to **Director**, e.g.

  <Tabs groupId="os-options">
    <TabItem value="windows" label="Windows" default>
      ```powershell
      vmetric-director -path="C:\Users\<user-name>\Documents\log-data.yml"
      ```
    </TabItem>
    
    <TabItem value="linux" label="Linux" default>
      ```bash
      vmetric-director -path="/usr/local/etc/log-data.yml"
      ```
    </TabItem>
...
Wherever you place your files, though, you have to keep in mind a few details while creating and editing configurations:

:::warning[attention]
In the YAML file where you will specify the components you intend to use, 

- the _kind_ of component you will declare (_devices_, _targets_, _pipelines_, or _routes_) has to be specified at the first level and followed by a colon, e.g. `devices:` or `pipelines:`

- the identifier of the component must be specified prefixed with a dash followed by a colon, e.g. `- id:` or `- name:`
:::

In other words, the YAML tree starts with the _kind_ of component you will enlist, the next level dashes indicate that it is an item of that kind, while the dashes on the third level indicate the enlisted values of a field that accepts arrays and maps:

```yaml
component-kind1:
  - item1-identifier: identifier1
    item1-field1: value
    item1-field2:
      - value1
      - value2
    ...
  - item2-identifier: identifier2
    item2-field1: value
    ...
component-kind2:
  - item1-identifier: identifier1
...
The **Schema** sections of the chapters for various kinds of components provide the fields used by that kind of component.
...
To stream and collect data points, to process them, and to forward them to destinations, **Director** uses the following types of configurable items.

### Devices

Devices are data sources that collect or receive data. The required fields are:
...
This is an HTTP device listening on port `8080`, and it expects the streamed data to be in JSON format.
...
Targets define the destinations where processed data is sent. The required fields are:
...
:::tip
You can use environment variables to specify your credentials.
:::
...
Pipelines describe data workflows that use processors which transform and enrich the data. The required fields are:
...
What types of data these metatags provide depends on the context and the log formats.
...
:::warning
Do _not_ confuse these fields with the component entries at the first level. As routes are at the highest level of a telemetry operation, they orchestrate it by specifying the components used to create the traffic.
:::
...
This route receives its data from devices with the prefix `syslog_` in their names, uses the `normalize` and `enrich` pipelines to extract and transform the received data points, and then forwards them to the target named `backup`.

:::note
For a general discussion of routes, see the [overview](/docs/configuration/routes/index.mdx).
:::

You can now try the examples to see how **Director** works.
