...
* `config/devices/syslog.yml`
* `config/routes/default.yml`
* `config/targets/console.yml`
* `config/targets/file.yml`
...
However, wherever you place your files, you have to keep one detail in mind while creating or editing the configuration:

:::warning[attention]
In the YAML file, the type of the component (_device_, _target_, _pipeline_, or _route_) has to be specified at the top level prefixed with a dash and followed by a colon, e.g. `- devices:`. Then, you can enlist the relevant details of the component, i.e. data fields, formats or patterns to used, etc.
:::
