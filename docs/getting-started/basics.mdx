...
- the kind of component you will declare (_devices_, _targets_, _pipelines_, or _routes_) has to be specified at the first level and followed by a colon, e.g. `devices:` or `pipelines:`
...
In other words, the first level entry indicates the _kind_ of the items you will enlist, the next level dashes indicate that it is an item of that kind, while the dashes on the third level indicate the enlisted values of a field that accepts arrays and maps:
