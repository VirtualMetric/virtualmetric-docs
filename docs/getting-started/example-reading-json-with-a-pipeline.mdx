...
- Using the [`json`](../configuration/pipelines/processors/json.mdx) processor, grab its contents and copy it to a field named `parsed_data` and&mdash;since we turned on its `add_to_root` setting&mdash;move the `words` field one level up.
