...
:::note
To run our pipeline, we can use the name we have specified for it. All we have to do is use the `mode` parameter to tell **Director** that we will run a "pipeline", and then `name` it on the CLI.
:::
