...
:::tip
When configuring the pipeline, devices are referred to with their _filenames_ whereas targets, routes, and other pipelines are referred to with the identifiers used in their `name` parameters.
:::
...
Next, we will create a target for **Microsoft Sentinel** in a file named `pipeline-target.yml`, again using the configuration we have created in our [target example](/docs/getting-started/example-target.mdx) as the basis:
...
Let's put it all together. We have created a device, a target, and a route to use in our pipeline, we included them in our pipeline configuration, and at this point we should have all our files ready.
