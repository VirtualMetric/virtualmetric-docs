...
Pipelines are used to grab data from producers, to reshape them with [processors](/docs/configuration/pipelines/processors/index.mdx), and then to send them to consumers.
...
To achieve this, we must have an environment with access to _Syslog_ messages. We also need a **Microsoft Sentinel** workspace with proper permissions.

Also, a basic understanding of the YAML format will be helpful in configuring the files.

For convenience, we will place all our files directly under `<vm_root>`.

:::note
You can place these files anywhere you wish under `config` since **Director** discovers them by iterating the folder tree. Your pipeline configuration file may be, e.g.

* `<vm_root>/config/pipeline.yml`
* `<vm_root>/config/routes/pipeline.yml`
...
Put this in a file named `pipeline-device.yml` under `<vm_root>`.
...
This configuration will parse the _Syslog_ header information using the `grok` processor and will extract authentication event details. It will then set the common event metadata. Next, using a script, it will categorize the events based on the type of `program` generating the log, and map the fields to the ASIM schema structure. Once done, it will clean up the temporary fields.
...
This routing configuration will connect to the _Syslog_ server as the source, apply the `syslog_to_sentinel` pipeline for processing the ingested data, and then send that data to the **Microsoft Sentinel** ASIM target we have specified.

## Monitoring

Let's put it all together. We have created a device, a target, and a route to use in our pipeline, we included them in our pipeline configuration, and at this point we should have these files ready as:
