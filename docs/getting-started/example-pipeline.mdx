...
# Example: Create A Syslog-To-Sentinel Data Pipeline
...
We will create a pipeline that ingests logs from a _Syslog_ source, processes them, and forwards them to **Microsoft Sentinel** in ASIM format.
...
First, we have to define a device to receive log data. We will use the configuration we have created in the device example as our starting, modifying some settings to fit our needs here:
...
Place this in a file named `pipeline-device.yml` under `<vm_root>`.
...
We are now ready to configure our pipeline. Create a file named `example-pipeline.yml` under the root, and place the following in it:
...
Now we will run our pipeline and see the results:
