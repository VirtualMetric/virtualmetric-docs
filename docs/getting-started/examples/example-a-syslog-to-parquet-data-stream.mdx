...
- parse the _Syslog_ header information using the [`grok`](../../configuration/pipelines/processors/grok.mdx) processor&mdash;reading the field `syslog.message` and extracting `event.action`, `user.name`, and `source.ip` from it;

- set the fields `event`, `user`, and `message` with the data extracted;
...
TODO: Check the output file. Screen shot.
