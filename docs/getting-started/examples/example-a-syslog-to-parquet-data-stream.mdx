...
We need to work in an environment that has access to _Syslog_ messages. We also need write permissions to the directory where the Parquet file will be stored, although since we will be using our [working directory](./overview.mdx#working-directory) that is taken care of.
...
With this configuration, we are setting up a _Parquet_ file target that will store the processed _Syslog_ data. We have specified batching and flush intervals for optimal performance, and enabled Snappy compression for efficient storage.
...
- Parse the _Syslog_ header information using the [`grok`](../../configuration/pipelines/processors/grok.mdx) processor&mdash;reading the fields `syslog.message`, `event.action`, `user.name`, and `source.ip`&mdash;and extract authentication event details
...
<Tabs>
  <TabItem value="powershell" label="PowerShell">
    ```PowerShell
    vmetric-director -pipeline -name=syslog_to_parquet -validate
    ```
  </TabItem>
