...
- **Ingest Syslog data** - Listen to _Syslog_ to receive messages.

- **Extract user related information** - Extract specific information from the _Syslog_ messages and map them to fields that will be stored in a _JSON_ file.

- **Write the data to a JSON file** - Write the processed information to a _JSON_ file.
...
- extract the `user`, `message`, `date`, and `time` values from it

- write the `user` value to the `user_name` field in the _JSON_ file

- using the [`concat`](../../configuration/pipelines/processors/concat.mdx) processor, combine the `message`, `date`, and `time` values and write the resulting string to the `event` field in the _JSON_ file

- remove the `message` field of _Syslog_ from the output
...
Note that we have placed our pipeline configuration within the route definition.

:::warning[attention]
Pipeline definitions should be either put in a separate file, or, if their configuration is included in the same file as the route, then they should be placed under the `pipelines:` branch of the route.
:::
