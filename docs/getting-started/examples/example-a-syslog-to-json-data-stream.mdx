...
- **Extract user related information** - Define the parameters to extract specific information from _Syslog_ data and map them to fields that will be stored in a _JSON_ file.
...
- **Write the data to a JSON file** - Define the parameters for sending the messages to a _JSON_ file as the destination.
...
With this configuration, we are setting up a _JSON_ file target that will store the processed _Syslog_ data. We have also specified batching and flush intervals for optimal performance, and enabled **ZTSD** compression for efficient storage.
  ...
  - name: extract_user_event
    processors:
      - grok:
          field: message
          pattern_definitions:
            SYSLOG_HEADER: "<\\d+> \\d+ \\S+ \\S+ \\S+ \\d+ \\S+ - "
            USER_NAME: "(?<User>[A-Za-z]+ [A-Za-z]+)"
            USER_MESSAGE: "(?<Message>.*)"
          patterns:
            - "%{SYSLOG_HEADER}%{USER_NAME} %{USER_MESSAGE}"            
      - remove:
          field: message
```

This configuration will parse the _Syslog_ header information using the [`grok`](../../configuration/pipelines/processors/grok.mdx) processor&mdash;reading the field `message` and extracting `SYSLOG_HEADER`, `USER_NAME`, and `USER_MESSAGE` from it, and send these to the _JSON_ file.
  ...
  - name: extract_user_event
    processors:
      - grok:
          field: message
          patterns:
            -  %{GREEDYDATA:user} %{GREEDYDATA:message_body} %{DATESTAMP:date} %{TIMESTAMP:time}
      - set:
          field: user
          target_field: user_name
      - concat:
          sources: ["Said '", message_body, "' on ", date, " at ", time]
          separator: ""
          target: event
      - remove:
          field: message
...
- using the [`grok`](../../configuration/pipelines/processors/grok.mdx) processor, parse the _Syslog_ header information to read the field `message`

- extract the `user`, `message_body`, `date`, and `time` value from it

- write the `user` value to the `user_name` field in the _JSON_ file

- using the [concat](../../configuration/pipelines/processors/concat.mdx) processor, combine the values of the `message_body`, `date`, and `time` and write the resulting text to the `event` field in the _JSON_ file

- remove the `message` field of _Syslog_
...
- apply the `extract_user_event` pipeline to the ingested data,
...
<Tabs>
  <TabItem value="powershell" label="PowerShell">
    ```PowerShell
    .\vmetric-director -pipeline -path=".\config\Examples\syslog-to-json.yml" -name=syslog_to_json -validate
    ```
  </TabItem>
  <TabItem value="bash" label="Bash">
    ```bash
...
Now, **Director** should be receiving _Syslog_ messages, processing them through our pipeline, and sending them to our _JSON_ file.

Press <kb-short>Ctrl+C</kb-short> to exit the processes on both terminals.


TODO: Copy 1- Screen output 2- JSON file contents

## Monitoring

Check the logs for any errors, and verify that the _JSON_ file was created with the processed _Syslog_ data.
