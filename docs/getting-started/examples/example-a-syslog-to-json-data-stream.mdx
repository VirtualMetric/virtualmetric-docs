...
We assume by now you understand the underlying logic of a pipeline since you have reviewed the <Topic id="example-pipeline-json">previous example</Topic>.
...
First, we have to define a device that will receive the log data. We will use the configuration in our <Topic id="example-ingesting">ingesting data</Topic> example since that will adequately serve our needs:
