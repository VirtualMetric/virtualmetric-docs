...
:::info
We need to set the devices's `status` to `true` to enable it.

**Director** allows you to add, update/modify, or remove devices and targets while it is running and, by the same mechanism, to start or stop them by enabling or disabling them.
:::
...
:::info
To see the ingested data, your must _route_ the stream to a specific destination! For a detailed discussion of Routes, see [this section](../../configuration/routes.mdx#configuration).
:::
...
Check **Director**'s logs&mdash;the files in the directory indicated [here](./user-notes.mdx#logging-setup)&mdash;for any error messages. If there were no errors, it should be blank.
