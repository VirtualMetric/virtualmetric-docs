...
<Tabs>
  <TabItem value="powershell" label="PowerShell" default>
    ```powershell
    Copy-Item -Path cert.pem key.pem -Destination <vm_root>
    ```
  </TabItem>
  <TabItem value="bash" label="Bash">
    ```bash
...
Note that we specified the certificate files we have created in the previous step. That is because we have set the `status` of the `tls` field to `true` so unless we point to these files the process will not run.
...
Time to test the secure connection. This time, however, we will send our messages using a standard command and piping it through `openssl`. 

:::info
On **Windows**, `openssl` is _not_ available by default. You have to install it yourself:
...
Our test message is sent to the port **Director** is listening to via TCP.
...
In the next section, we will create a different type of _target_ to forward the data ingested through the UDP device we have defined here.
