...
Create a file named `from-syslog-udp.yml` in our [working directory](./overview.mdx#working-directory), and then open it with a text editor to enter the following:
...
We can now test the reception using either **Director**'s cross-platform `generator` mode or opther traditional tools available on your platform. Open a new terminal and enter the following in the command line:
...
Time to test the secure connection. In the terminal where you have previously sent messages in `generator` mode, and enter the following command:
