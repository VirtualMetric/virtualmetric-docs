...
Assume, however, that we want to customize the stream directed to the console. For that purpose, we will modify the target configuration in our `to-console.yml` file (placed in our [working directory](./overview.mdx#working-directory)) as:
...
Then, once again, go to the other terminal you are using for the `generator` mode (or other traditional message sending tool), and start sending our familiar message:
...
To be able to use this target, we have to configure another route. To keep things simple, first delete any previously created route definition.

:::note
**VirtualMetric Director** runs all the defined routes it has spotted under the `config` directory simultaneously.
:::

Then, create a file named `syslog-to-json.yml` and place this new route definition in it:
