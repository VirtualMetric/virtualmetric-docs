...
### Step 3: Send messages to console
...
:::info
You can create new configuration files, or update existing ones, as **Director** is running in the background. After being invoked with the `-console`, it will print an update status every time a configuration is added or modified.
:::
...
### Step 4: Send messages to output file
...
Check the log file indicated [earlier](./overview.mdx#logging-setup). If there were no errors during the trial, it should be blank.
