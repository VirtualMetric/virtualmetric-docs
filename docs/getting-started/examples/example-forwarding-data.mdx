...
:::tip
For a detailed discussion of targets, see [this section](../../configuration/targets/overview.mdx#configuration).
:::
...
Filenames do not include the path to avoid variation in syntax for different platforms. All configuration files will be placed in our working directory.
:::
...
Recreate this file and put this configuration in it.

Note that we added a `format` property. With this setting, we will normalize our data to the [ECS](../../appendix.mdx#ecs) format which provides a standardized, consistent structure to query Web data.

To see the results on the console, once again go to the terminal you are using for **Director**, and invoke it by entering:
...
Then, switch to the other terminal you are using for the `generator` mode (or other traditional message sending tool), and start sending our familiar message:
...
<Tabs>
  <TabItem value="powershell" label="PowerShell" default>
    ...
    ```powershell
    .\vmetric-director -generator -mode=syslog -address="127.0.0.1:514" -message="Hello world"
    ```
  </TabItem>
  <TabItem value="bash" label="Bash">
    - Using `generator` mode
...
<Tabs>
  <TabItem value="powershell" label="PowerShell" default>
    ...
    ```powershell
    .\vmetric-director -generator -mode=syslog -count=1 -address="127.0.0.1:514" -message="Hello world"
    ```
  </TabItem>
  <TabItem value="bash" label="Bash">
    - Using `generator` mode
...
To have this _Syslog_ message directed to the storage file we have configured, invoke **Director** from your other terminal:
