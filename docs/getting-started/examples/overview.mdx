...
Deleting the file here is harmless. So long as the above debugging setting remains, it will be re-created with each run.

## Sending Messages

As **VirtualMetric DataStream** is a telemetry system that captures, processes, and forwards log emssages, we will have to send those messages to our sample configurations by some mechanism. In this guide, we will be utilizing the generator mode of **Director**, which needs to be run on a separate terminal and requires the use of a syntax similar to the following:

> `.\vmetric-director -generator ...`

You are free to use other traditional tool&mdash;such as `logger` for **Unix**-based on platforms. Only make sure that it is not invasive, and only sends messages to the listeners on the ports specified.
