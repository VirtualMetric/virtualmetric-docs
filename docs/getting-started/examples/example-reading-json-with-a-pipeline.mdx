...
:::info
For a detailed discussion of pipelines, see <Topic id="pipelines-overview-config">this section</Topic>.
:::
...
- Using the <Topic id="processors-json">`json`</Topic> processor, grab its contents and&mdash;since we turned on its `add_to_root` setting&mdash;move the `words` field one level up.
- Using the <Topic id="processors-uppercase">`uppercase`</Topic> processor, convert the words to uppercase and write them to a field named `converted_words`.
...
:::info[important]
Note that we have to specify the paths and names of the files we are using. This is specific to the _pipeline_ mode.
:::
