...
<Details summary="Install OpenSSL">
  ...
  4. Add the installation location to your `$PATH`&mdash;if you did not change the default installation directory, this will be:
...
<Include id="third-party-disclaimer" />
...
Now we have to convert this  PFX file to the <Topic id="appendix-pem">PEM</Topic> format that **Director** uses. To do that, we first have to copy the `cert.pfx` file to `<vm_root>` and navigate there:
...
This will start **Director** as a background process which you can verify as <Topic id="single-node-deployment-director-post">indicated before</Topic>. Now enter:
...
- No error files in `<vm_root>/Director/storage/logs/`
