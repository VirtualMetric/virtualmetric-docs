...
FIXME: To be corrected from here on.
...
The IP number `127.0.0.1` is the one that is assigned to the local machine by default. If the result you get is

<Tabs>
  <TabItem value="powershell" label="PowerShell" default>
    ```powershell
    WARNING: TCP connect to (127.0.0.1 : 6514) failed
    ```
  </TabItem>
  <TabItem value="bash" label="Bash">
    ```bash
