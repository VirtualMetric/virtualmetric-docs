...
- **Install Agent** - Deploy the Agent service on a remote system that needs monitoring.
...
We assume you have a **Director** instance running and accessible from the Agent system. The Agent will need network connectivity to reach the Director's service endpoint.
...
- Create a device entry with `id: 1` for the Agent to connect to
...
- Include two metric `definitions` that specify which system metrics to collect
...
On the remote system, launch the **Agent**:
...
In the Director console, you should see messages indicating successful Agent connection and data collection:
...
If the Agent is running correctly, you should see periodic metric collection messages in the Director console, indicating the distributed monitoring system is operational.