...
Assume, however, that we want to customize the stream directed to the console. For that purpose, we will modify the target configuration in our `to-console.yml` file (placed in our [working directory](./configuration-basics.mdx#working-directory)) as:
...
To see the results on the console, once again go to the terminal you are using for **Director** invoke it it by entering:
...
To be able to use this target, we have to configure another route. But, to keep things simple, first open the `syslog-to-console.yml` and comment out the route definition in it, and save:
...
Now we can check our ouput. In our working directory, you will find a file with a name like `from_syslog_2025_06_02.json`. Open it with a text editor. You will find that the ingested data has been stored in it in a format like this:
