...
Note that we specified the certificate files we have created in the previous step.
...
<Tabs groupId="os-options">
  <TabItem value="windows" label="Windows" default>
    ```powershell
    vmetric-director -path=secure-syslog.yml
    ```
  </TabItem>
...
We can also run it in the background:
