...
In this chapter, we will see how to configure a device to ingest data with specific protocols.

The device configuration files have a `.yml` extension, and they reside inside the `devices` directory under the `config` folder:
...
However, users can organize their device files as they wish, regardless of the above folder structure. This can be done in two ways:
...
The number of workers, the buffer size, and the flush interval will all have ab impact on performance.
