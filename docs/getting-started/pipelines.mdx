...
Place this file in the `config` directory of `<vm_root>`. This location ensures that **Director** will automatically detect and load your configuration at startup.
...
This Kafka device connects to a broker at `kafka:9092` and consumes messages from the `incoming-data` topic. The preprocessing section defines normalization logic that ensures the timestamp is in ISO 8601 format and converts the value field to a floating-point number.
