...
**Director** is a single binary that requires no dependencies, so you can easily download and place it anywhere on your system. It can be installed on a traditional server, on an on-premises virtual machine, or in the cloud. It can also be deployed as an <Topic id="deployment-azure-app-service">**Azure App**</Topic> for a serverless configuration.
