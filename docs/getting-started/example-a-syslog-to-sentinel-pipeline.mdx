...
First, we have to define a device that will receive the log data. We will use the configuration we have created in our [ingesting data example](../getting-started/example-ingesting-data.mdx) as our starting point, modifying it as fits our needs:
...
Next, we will define a target for **Microsoft Sentinel** in our file, again using the configuration we have created in our [forwarding data example](../getting-started/example-forwarding-data.mdx) as the basis:
