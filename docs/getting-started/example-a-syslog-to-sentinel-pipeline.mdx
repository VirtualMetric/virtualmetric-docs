...
For this, we will implement the following stages:

|Step|Purpose|
|---|---|
|Create the device|Define the parameters of a `syslog` listener to receive messages. See the fields available in the [**Syslog**](../appendix.mdx#syslog) format|
|Create the target|Define the format for sending the messages to `sentinel` as the destination. See the fields available in the [**ASIM**](../appendix.mdx#asim) format|
...
|Define the route|Specify the source of data (i.e. the _devices_), the destination of the data (i.e. the _target_), and the _pipeline_ to be used to transform the data|
...
:::tip
When configuring a pipeline, devices are referred to with the numbers in their `id` parameters, whereas targets, routes, and other pipelines are referred to with the identifiers in their `name` parameters.
:::
...
We are now ready to configure our pipeline. Put the following in the configuration file:
...
<Tabs>
  <TabItem value="windows" label="Windows">
    ```PowerShell
    .\vmetric-director.exe -mode=pipeline -name=syslog_to_sentinel -validate
    ```
  </TabItem>
