...
:::note
Under the [next heading](#the-configuration-files), you can find the details of the configuration of these components.
:::
...
To illustrate, in order to stream data in _Syslog_ format, the device configuration YAML&mdash;arbitrarily named `basic-syslog.yml` here&mdash;should contain at least these settings:
...
The `name` specified, i.e. `basic_syslog` is also arbitrary and for the user to choose.
