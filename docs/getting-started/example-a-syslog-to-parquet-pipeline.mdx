...
If our pipeline is valid, this command returns `true`. Since that is indeed the case, we can now run our test.
...
Here is what this test should do:

* Consume the data in the input file named `sample-data.json` we put in our working directory
...
For details of the file layout, see the [Parquet](../appendix.mdx#parquet-files) format.
