...
1. **Create a device** - Define the parameters of a _Syslog_ listener to receive messages.

2. **Create a target** - Define the parameters for sending the messages to a _Parquet_ file as the destination.

3. **Create a pipeline** - Define the parameters to extract some _Syslog_ data and map them to fields to be stored in the _Parquet_ file.

4. **Create a route** - Define the parameters that will relate the _source_ to the _pipeline_ that will process the ingested data and the _destination_ that will receive the processed result.

## Setup

We need to work in an environment that has access to _Syslog_ messages. We also need write permissions to the directory where the Parquet file will be stored, although since we will be using our [working directory](./configuration-basics.mdx#working-directory) that is taken care of.
...
- Copy the values it has spotted in that field to the root of the output document using the `json` processor's `add_to_root` flag.
- Using the [`foreach`](../configuration/pipelines/processors/foreach.mdx) processor&mdash;which applies the specified processor (i.e. [`uppercase`](../configuration/pipelines/processors/uppercase.mdx)) to each element in the list returned by the meta field `"_ingest._value"`&mdash;convert each item to uppercase.

### Validate Pipeline

To see whether our pipeline is valid, we will enter the following command in the terminal:

<Tabs>
  <TabItem value="windows" label="Windows">
    ```PowerShell
    vmetric-director -mode=pipeline -name=convert_words -validate
    ```
  </TabItem>
...
If our pipeline setup is valid, this command should return `true`. If that is indeed the case, we can now run our test.
...
<Tabs>
  <TabItem value="windows" label="Windows">
    ```PowerShell
    vmetric-director -mode pipeline -name convert_words -input ".\config\Examples\sample-data.json" -output ".\config\Examples\processed-sample-data.json"
    ```
  </TabItem>
  <TabItem value="linux" label="Linux">
    ```bash
...
* Using the pipeline named `convert_words`, convert the names in the `words` field to `uppercase`
...
{
  "names": ["hello", "world"],
  "convertedNames": ["HELLO", "WORLD"]
}
```

Now we can proceed to creating our end-to-end data flow with a device, a pipeline, and a target for storage.
...
See the [Syslog](../appendix.mdx#syslog) format for available fields.
...
See the [Parquet](../appendix.mdx#parquet-files) format for the file layout.
...
This configuration will connect to the _Syslog_ server, apply the `parse_syslog` pipeline to the ingested data, and then write it to the _Parquet_ file specified.
