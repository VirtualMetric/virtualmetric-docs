...
- `491591344758837249` is base definition id
...
You need to use only one of these YAML files. Delete the example you do not want to use.
...
Download `vmetric-agent.exe` to a folder. Then create a `vmetric.yml` in the same folder, and add the following basic configuration information.

id will be the same that is configured in director devices section. director address is the ip address for director service server.
