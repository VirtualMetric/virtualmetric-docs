...
There are 3 different security definitons that can be used under the `director\package\definitions\module\windows\host\event` directory.
...
You need to use only one of these YAML files. Delete the example from the one that you want to use.
