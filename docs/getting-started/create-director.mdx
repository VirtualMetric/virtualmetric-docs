...
2. **Start Director Creation**
   - Click the blue <gui>Create director</gui> button
   - You'll see the **Director Setup** form

3. **Configure Basic Settings**
   - **Name**: Enter a unique name (e.g., "My First Director")
   - **Choose installation type**: Select <gui>Standalone</gui>
...
1. **Choose Your Environment**
   - **Cloud**: Azure VM, Amazon EC2, Google Compute Engine, etc.
   - **On-premises**: Physical server, virtual machine, container platform
   - **Requirements**: See <Topic id="directors-deployment">Deployment Guide</Topic> for detailed requirements

2. **Get Installation Script**
   - The interface shows platform-specific installation scripts
   - Choose <gui>PowerShell</gui> (Windows) or <gui>Bash</gui> (Linux/macOS)
...
4. **Verify Connection**
   - Click the <gui>Verify connection</gui> button in the **DataStream** interface
   - Wait for the "Connection successful" message
   - If connection fails, check network connectivity and firewall settings

5. **Complete Setup**
   - Click <gui>Complete setup</gui> to finish the process
...
- Try the <gui>Re-install director</gui> option if needed
