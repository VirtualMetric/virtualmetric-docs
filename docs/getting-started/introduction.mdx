...
- Download and install **Director** using [this section](./single-node-deployment-director.mdx).
...
- To simulate streaming data, we provide **Director**'s companion utility **Generator**. It can be used to send messages to specific logging services, although other traditional tools&mdash;such as `logger`&mdash;can also be used for this purpose on some **Unix**-based platforms.
	...
	Make sure to download and install **Generator** using [this section](./single-node-deployment-generator.mdx) since will be using it in our introductory examples.

- Download and install **Agent** using [this section](./single-node-deployment-agent.mdx).

We recommend that you keep all three executables in the same directory, preferably named `VirtualMetric`. In our examples throughout the documentation, we will assume that `<vm_root>` is:
...
This is to avoid surprises like the system adding another extension to your YAML file's extension, e.g. `file.yml.txt`, in which case **VirtualMetric** executables will not be able to recognize them.
:::
...
- [**Example: Readon JSON With a Pipeline**](./example-reading-json-with-a-pipeline.mdx) - Configure your first pipeline to receive and process data
