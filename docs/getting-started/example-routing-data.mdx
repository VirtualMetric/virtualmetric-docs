...
:::note
As our target, we are using the **Parquet** storage we have configured in the [previous example](./example-forwarding-data.mdx#storage-file-setup).
:::

As the syntax indicates, multiple routes can be specified: the same source data can be forwarded to multiple destinations, or a destination may receive data from multiple sources.

Here we are sending the data to a single storage file.

## Pipeline Use

A route can use pipelines as part of its operation. Multiple pipelines can be specified to apply to various streams of data.
...
This configuration will [normalize](../configuration/pipelines/normalization.mdx) the data before sending it to our target.
...
The specified 3 pipelines are used for purposes that should be obvious from their names: normalizing, enriching, and aggregating. The data will then be routed to our Parquet file.
...
routes:
  ...
  - name: windows_route
    ...
    targets:
      - name: windows_to_parquet
```

These routes will collect two streams of data, one from a _syslog_ and one from a _windows_ device, normalize them using the specific pipelines of each, and then direct them to their respective Parquet file targets.

:::note
This time we have specified two different targets&mdash;`syslog_to_parquet` and `windows_to_parquet`&mdash;which have to configured in their own YAML files. We leave that as an exercise to the reader.
:::
...
Here, the data will be received by 3 different Parquet files: a primary storage file, a backup storage file, and an analytics platform file.
