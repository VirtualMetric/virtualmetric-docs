...
Create a file named `from-syslog-udp.yml` in our [working directory](./configuration-basics.mdx#working-directory), and then open it with a text editor to enter the following:
...
However, we also need to define a target that the ingested data willl be directed to, and a route combining the device and the target. Create a file named `to-console.yml` and add the following to it:
...
Finally, create a file named `syslog-to-console.yml` and add the following route definition to it.
...
We can now test this using either **Director**'s cross-platform companion **Generator** or the traditional tools available on your platform. Open a new terminal and enter the following in the command line:
...
If you are using **Generator**, after you press <kb-short>Enter</kb-short> the program will pause for a few seconds, and then it will start sending our message to _Syslog_. It will signal that it started sending messages with a prompt like this:

<Tabs>
  <TabItem value="windows" label="Windows" default>
    ```PowerShell
    1000 messages sent successfully in 11 ms with 0 error(s) at 2025-06-02 01:54:02.0349011 +0300 +03 m=+2.821324601
    2000 messages sent successfully since 2025-06-02 01:54:00 +0300 +03
    ```
  </TabItem>
  <TabItem value="linux" label="Linux">
    ```bash
...
If you now switch back to **Director**'s terminal, you will see the message sent (i.e. "Hello world") on the console:

<Tabs>
  <TabItem value="windows" label="Windows" default>
    ```PowerShell
    <3> 2025-06-02T08:33:28+03:00 VirtualMetric Test[3872]: Hello world
    ```
  </TabItem>
  <TabItem value="linux" label="Linux">
    ```bash
