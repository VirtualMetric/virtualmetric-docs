...
<Tabs>
  <TabItem value="powershell" label="PowerShell" default>
    ```PowerShell
    Copy-Item -Path C:\Path\To\Downloads\vmetric-agent.exe -Destination "C:\Program Files\VirtualMetric"
...
<Tabs>
  ...
  <TabItem value="bash" label="Bash">
    ```bash
    sudo ./vmetric-agent -service install
    sudo ./vmetric-agent -service start
    ```
  </TabItem>
</Tabs>
      ...
      ● vmetric-agent.service - VirtualMetric Agent handles the monitoring of local and remote devices.
        ...
        Main PID: 545 (vmetric-agent)
          ...
          CGroup: /system.slice/vmetric-agent.service
                  └─545 /opt/virtualmetric/vmetric-agent -mode=agent
...
<Tabs>
  ...
  <TabItem value="bash" label="Bash">
    ```bash
    sudo ./vmetric-agent -service stop
    ```
  </TabItem>
</Tabs>
