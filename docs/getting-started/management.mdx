...
The configurations may be placed in separate files or they may be grouped together in common files according to what suits your requirements.

## Configuration Files

By default, device configuration files reside in the `devices` directory under the `config` folder and have a `.yml` extension:
