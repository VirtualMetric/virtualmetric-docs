...
We are revolutionizing the traditional concept of _source_ in telemetry pipelines with its introduction of _Datasets_.
...
Unlike other solutions that focus solely on data collection via protocols or third-party agents, _Datasets_ categorize telemetry data&mdash;i.e. Windows Event Logs, Windows User Activity, Linux Audit Logs, etc.&mdash;at the source, simplifying pipeline design and enabling advanced RBAC.
...
We have simplified data routing with our advanced `reroute` processor, eliminating the need for manual filtering required in other solutions.
...
graph TD
    ...
    subgraph Processors[50+ Processors]
        P1[Parsing]
        P2[Filtering]
        P3[Enrichment]
        P4[Routing]
...
Whatever your telemetry operational needs, **DataStream** has something to offer to make your operation more streamlined and efficient, more robust, and more secure at drastically reduced costs.
