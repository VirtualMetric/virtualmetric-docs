---
pagination_prev: null
---

# Glossary

## A

<dt>**Audit**</dt>
<dd>
	Review of logs for the purpose of verifying that IT policies are correctly implemented
</dd>

## B

<dt>**Batch Processing**</dt>
<dd>
	Collective sequential processing of a large number of records that require the same operation
</dd>

## E

<dt>**Event**</dt>
<dd>
	Any occurence in a computerized and networked system that registers as a sender or receiver of signals
</dd>

## I

<dt>**Ingestion**</dt>
<dd>
	Collection of data from a source for specific purpose of use
</dd>

## M

<dt>**Management**</dt>
<dd>
	The policy implemented to handle log lines after they have been processed
</dd>

## N

<dt>**Normalization**</dt>
<dd>
	The process of transforming data points with the intention of rendering them more suitable for use by a data consumer
</dd>

## P

<dt>**Parquet File**</dt>
<dd>
	An open source, columnar storage file format
</dd>

<dt>**Pipeline**</dt>
<dd>
	Pre-configured processing logic for a data stream
</dd>

<dt>**Processor**</dt>
<dd>
	A component that performs a specific operation on streaming data
</dd>

## R

<dt>**Routing**</dt>
<dd>
	Sending or directing a data stream to a destination for further processing or analysis
</dd>

## S

<dt>**Supervisor**</dt>
<dd>
	The component that checks the health of the processes. It restarts stopped services, cleans up the `temp` folder, etc.
</dd>

## T

<dt>**Template**</dt>
<dd>
	Container object with selection and/or processing logic for an incoming data stream
</dd>

## V

<dt>**Vectorized Processing**</dt>
<dd>
	Some sort of processing
</dd>
