...
* **Cloud Integration** - These devices connect to cloud services:
  * **AWS S3**: Processes files from Amazon S3 buckets using SQS event notifications
  * **AWS Security Lake**: Consumes OCSF Parquet files from Amazon Security Lake via SQS notifications
