...
|Field|Type|Required|Default|Description|
|---|---|---|---|---|
|`region`|string|Y|-|Amazon region where Security Lake and SQS queue reside|
|`queue_url`|string|Y|-|SQS queue URL for S3 event notifications|
|`access_key_id`|string|Y*|-|Amazon access key ID for authentication|
|`secret_access_key`|string|Y*|-|Amazon secret access key for authentication|
|`session_token`|string|N|-|Amazon session token for temporary credentials|
...
Avoid hardcoding `secret_access_key` in plain text. Prefer environment variables, Amazon Secrets Manager, or IAM roles for production deployments. Follow Amazon least-privilege security principles.
:::
...
Amazon enforces strict SQS parameter limits. Values outside allowed ranges are automatically clamped to boundaries.
:::
...
**Cross-Account Access**: When consuming Security Lake data from a different Amazon account, use IAM role assumption with `role_arn` and `external_id`. The device automatically handles temporary credential rotation based on `role_session_timeout`.
