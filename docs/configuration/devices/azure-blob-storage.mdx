...
|`description`|string|-|-|Optional description of the device's purpose|
|`type`|string|Y|-|Device type identifier (must be `azblob`)|
|`tags`|string[]|-|-|Array of labels for categorization|
|`pipelines`|pipeline[]|-|-|Array of preprocessing pipeline references|
|`status`|boolean|-|true|Boolean flag to enable/disable the device|
