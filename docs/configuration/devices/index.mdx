...
* **Standalone** - With this approach, sources are connected directly to **Director**, bypassing any intermediate mechanism.
...
* **High Availability** - With this approach, sources are first connected to a load balancer&mdash;which distributes the work to multiple processes&mdash;after which they are streamed to either a **Director** cluster:
...
The fields required to define a device are:

- `id`: Unique numeric identifier for the device
...
This is an HTTP device listening on port `8080`, and it expects data in JSON format.
...
| `name` | Y | Human-readable device name |
...
These devices listen for incoming network connections:
...
Simple UDP listener for network data:

```yaml
- id: 1
  name: basic_udp_collector
  type: udp
  properties:
    port: 514
```
