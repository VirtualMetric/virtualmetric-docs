...
Scheduling applies to both Director-based and Agent-based processing, enabling time-based control regardless of where pipeline execution occurs.
...
- **Agent Processing**: Control when Agents process data with pipelines before transmission
...
**Learn more:** <Topic id="cron-based-scheduling">Cron Documentation</Topic>
...
**Learn more:** <Topic id="interval-based-scheduling">Interval Documentation</Topic>
...
### Agent Pre-Processing with Scheduled Execution

Control when Agents process and transmit data:

```yaml
# Agent device with scheduled pre-processing
devices:
  - name: windows_agent_01
    type: windows
    properties:
      # Agent processes logs every 15 minutes
      interval: "15m"
      pipeline: "filter_and_mask"

# Agent device with cron-based processing
devices:
  - name: windows_agent_02
    type: windows
    properties:
      # Agent processes logs during business hours only
      cron: "* 9-17 * * 1-5"
      pipeline: "compliance_transform"

# Agent device with realtime processing
devices:
  - name: windows_agent_03
    type: windows
    properties:
      # No interval - continuous processing
      pipeline: "realtime_filter"
```

**Benefits:**
- Reduce Agent resource consumption through scheduled processing
- Control network bandwidth by batching data transmission
- Align Agent processing with business hours or maintenance windows
- Distribute processing load across time for multiple Agents

**Use Cases:**
- **Edge Filtering**: Process and filter logs every 5 minutes at collection point before transmission
- **Off-Hours Processing**: Run expensive Agent transformations during non-business hours
- **Compliance Windows**: Ensure regulatory transformations occur during specific time windows
- **Cost Management**: Batch Agent transmissions to reduce network and Director processing costs
...
- **Agent pre-processing**: Use scheduled Agent processing to distribute load and reduce network bandwidth
...
**Next Steps:**
- Read <Topic id="cron-based-scheduling">Cron Documentation</Topic> for cron-based execution
- Read <Topic id="interval-based-scheduling">Interval Documentation</Topic> for interval-based execution
