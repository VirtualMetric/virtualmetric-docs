...
Cron-based execution allows targets, routes, and Agent devices to run at specific times using cron expressions. Instead of running continuously, the component will only process data when the cron schedule condition is met.
...
The cron field accepts standard cron expressions to control when a target, route, or Agent device executes. This is useful for scenarios where you want to:
...
- Control when Agents process logs with pipelines before transmission
...
For Agent devices with pre-processing pipelines, cron scheduling controls when the Agent executes its pipeline transformations and transmits processed data to the Director.
