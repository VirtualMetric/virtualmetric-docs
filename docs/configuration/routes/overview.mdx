...
Routes are the **orchestration layer** in the DataStream processing flow. They bind devices, pipelines, and targets into complete processing workflows, determining what data goes through which pipelines and reaches which destinations.

> Provider → Device → Preprocessing → Pipeline → Postprocessing → Target → Consumer
>
> **Route** _(orchestrates the entire flow)_

<Image id="routes-flow" />
...
DataStream provides two approaches for creating routes:

**Quick Routes** use a visual drag-and-drop interface for simple device-to-target connections with optional pipeline processing. Best for straightforward data flows.

**Advanced Routes** use a multi-step wizard supporting complex filtering, conditional logic, Director selection for targeted execution, and scheduling for batch processing. Best for enterprise routing scenarios.

Both methods produce the same underlying YAML configuration. The GUI approach is covered in the Routes Management documentation.
...
|`pipelines`|N||List of pipelines to process matching events (processed in parallel)|
