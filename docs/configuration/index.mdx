...
To help design the logic of the components that will define, combine, and run the processes of the above-mentioned stages, **Director** uses configuration files that can be found in the folders under `vm_root`:
...
The configurations may be placed in separate files or they may be grouped together logically, i.e. based on how they are intended to be used or the type of data streams they process. By default, these files reside in the directories under the `config` folder, and have a `.yml` extension. They are organized as, e.g.:
...
It is also possible to use _standalone_ files placed in any arbitrary directory you wish. In that case, the full path of the file must be supplied to **Director**.

Select the organizational style that best suits your needs.

## Definition Syntax

All configuration files must conform to the following BNF rules:

```bnf
      component-declaration ::= <component-type> ":" "\n\t" <component-definition>
             component-type ::= ("devices" | "targets" | "pipelines" | "routes")
       component-definition ::= <identifier-field-definition> <field-definitions>

identifier-field-definition ::= "-" <identifier-field-name> ":" <identifier>
      identifier-field-name ::= ("id" | "name")
                 identifier ::= <text-value>

          field-definitions ::= <field-definition> ["\n" <field-definitions>]*
           field-definition ::= <field-name> ":" <field-values>
               ...
               field-values ::= (
                ...
                field-value ::= (text-value | numeric-value)

                 text-value ::= <text-character>+ <alphanumeric-character>*
              numeric-value ::= ('-' | '+')? <numeric-character>+
     alphanumeric-character ::= (<text-character> | <numeric-character>)*
             text-character ::= ('a' .. 'z' | 'A' .. 'Z' | '_')
...
* **System** - Monitoring and tracking performance metrics, monitoring resource usage, reviewing error logs, and generating reports
