...
{/* Schematically, a data stream leading to various pipelines are likely to be structured like this:

<img src={useBaseUrl('/img/graphs/device-to-pipeline.svg')} alt="Device To Pipeline" height="150"/>

and the processor chain will frequently resemble this:

<img src={useBaseUrl('/img/graphs/processor-chain.svg')} alt="Processor Chain" height="50"/> */}
...
Pipelines can be referenced in device configurations or routes:
...
:::note
When multiple pipelines are attached to a **device**, they execute sequentially in the order listed. When multiple pipelines are attached to a **route**, they execute in parallel. Processors within each pipeline always execute sequentially.
:::
