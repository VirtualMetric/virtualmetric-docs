---
description: Adds an explanatory comment
sidebar_custom_props: 
  customIcon: 💭
  customCategory: Text Processing
---

# Comment

<span className="theme-doc-version-badge badge badge--secondary">Cribl Compatible</span>

## Synopsis

Adds a comment to the pipeline configuration for documentation purposes.

## Schema

```yaml {2}
- comment:
    message: <string>
    description: <text>
    tag: <string>
```

## Configuration

The following fields are used to define the processor:

|Field|Required|Default|Description|
|---|---|---|---|
|`message`|Y|-|The comment text to include|
|`description`|N|-|Additional explanatory note|
|`tag`|N|-|Identifier for the comment|

## Details

The processor does not modify any data, and is used purely for adding explanatory notes.

:::note
Comments are preserved in the pipeline configuration but have no impact on data processing.
:::

Comments can help document the pipeline logic, business rules, data transformations, configuration rationale, and implementation details.

:::tip
Use meaningful tags to organize and categorize comments for better pipeline management.
:::

## Examples

### Basic

<ExampleGrid>
  <CommentCol>
    A simple explanatory pipeline comment:
  </CommentCol>
  <CodeCol>
    ```yaml
    - comment:
        message: "Process only high severity events"
```
  </CodeCol>
</ExampleGrid>

### Complex Logic

<ExampleGrid>
  <CommentCol>
    Documenting complex rules is good practice:
  </CommentCol>
  <CodeCol>
    ```yaml
    - comment:
        message: "Apply specific processing for production environment"
        description: "Production requires additional validation steps"
        tag: "env-specific"
```
  </CodeCol>
</ExampleGrid>

### Section Headers

<ExampleGrid>
  <CommentCol>
    Comments can be used as section dividers:
  </CommentCol>
  <CodeCol>
    ```yaml
    - comment:
        message: "=== Data Enrichment Section ==="
        tag: "section-header"
```
  </CodeCol>
</ExampleGrid>

### Implementation Notes

<ExampleGrid>
  <CommentCol>
    Implementation details can be highlighted:
  </CommentCol>
  <CodeCol>
    ```yaml
    - comment:
        message: "To do: Add support for custom field mapping"
        description: "Pending feature implementation in v2.0"
        tag: "todo"
  ```
  </CodeCol>
</ExampleGrid>
