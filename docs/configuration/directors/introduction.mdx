...
**Clustered** installations provide high availability and horizontal scaling capabilities through multiple Director instances working together. A cluster supports automatic load balancing, built-in failover and redundancy mechanisms, and horizontal scaling based on processing demands. Directors in a cluster share state management across nodes and distribute processing for improved performance.

Clustering eliminates single points of failure, improves processing capacity and throughput, and enables automatic recovery from node failures. Clusters scale dynamically based on data volume and provide enhanced monitoring and observability across all nodes.
