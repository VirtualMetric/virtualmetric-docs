...
:::warning
Directors already assigned to another cluster are not available for selection. Remove a Director from its current cluster before reassigning.
:::
...
- **Certificate Name** - TLS certificate filename (required if TLS enabled)
- **Key Name** - TLS private key filename (required if TLS enabled)
...
- **Token Type** - Always "cluster" for cluster-level tokens
- **Expiration**:
  - 1 month
  - 3 months
  - 1 year
...
- **Certificate Name** - Current certificate file
- **Key Name** - Current key file
