...
For redundancy, group **Directors** into clusters for automatic failover and load balancing. Clusters require a minimum of 3 Directors (odd number for quorum) and provide continuous operation when individual Directors fail. See the [Clusters](#clusters) section in <Topic id="directors-management">Directors: Management</Topic> for configuration details. Consider geographic distribution for disaster recovery and plan for seamless failover procedures.
