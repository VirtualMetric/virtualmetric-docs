...
Every component requires a unique identification:
...
Sensitive information can be stored in system variables:
...
Components can be tagged for better organization, and&mdash;optionally&mdash;a description can be used to document its purpose:
...
### Scoping and Reference

Fields within the same YAML file are all part of the same scope, and they can be referred to by other fields. This can be done in two ways:
...
2. Using the so called _mustache_ syntax for concatenation:

    ```yaml
    processors:
      - set:
          field: some_field
          value: "User {{user_name}} sent {{file_name}}"
    ```
