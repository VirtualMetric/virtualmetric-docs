...
- **Status Control** - Components can be enabled or disabled using the `status` field by using `true`/`false`:

- **Environment Variables** - Sensitive information such as `${ADMIN_PASSWORD}` can be stored in system variables:
