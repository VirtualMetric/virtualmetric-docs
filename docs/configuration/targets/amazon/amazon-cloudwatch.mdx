...
<span className="theme-doc-version-badge badge badge--secondary">Amazon</span><span className="theme-doc-version-badge badge badge--secondary">Logging</span>

## Synopsis

Creates a target that writes log messages to Amazon CloudWatch Logs with support for batching and Amazon authentication. The target handles message delivery efficiently with configurable batch limits.
...
|`type`|Y||Must be `amazoncloudwatch`|
...
CloudWatch Logs retains log data indefinitely by default. You can configure retention periods from 1 day to 10 years at the log group level through the Amazon Console or API.
