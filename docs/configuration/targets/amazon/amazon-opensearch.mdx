...
<span className="theme-doc-version-badge badge badge--secondary">Amazon</span><span className="theme-doc-version-badge badge badge--secondary">Search & Analytics</span>

## Synopsis

Creates an Amazon OpenSearch target that sends data using the Bulk API with Amazon IAM authentication. Supports multiple endpoints, field normalization, customizable batch sizing, and automatic load balancing across OpenSearch nodes.
...
|`use_iam`|N|`true`|Use Amazon IAM authentication (recommended)|
|`region`|Y*|-|Amazon region for IAM authentication|
|`key`|N*|-|Amazon access key ID|
|`secret`|N*|-|Amazon secret access key|
|`session`|N|-|Amazon session token for temporary credentials|
|`username`|N|-|Basic auth username (alternative to IAM)|
|`password`|N|-|Basic auth password (alternative to IAM)|

\* = Required when `use_iam` is `true`. If `key` and `secret` are not provided, the target will use the default Amazon credential chain (environment variables, IAM role, etc.).
...
The target supports Amazon IAM authentication using Signature V4, multiple endpoints, compression, and ingest pipelines. Data is batched for efficient delivery and can be automatically routed to different indices.
...
- Uses Amazon Signature V4 to sign HTTP requests
- No passwords stored in configuration
- Integrates with Amazon IAM policies and roles
...
When `use_iam` is `true`, the target signs all HTTP requests with Amazon Signature V4. The signing process uses the service name `es` as required by Amazon OpenSearch Service.

### Amazon Credential Chain

If `key` and `secret` are not provided in the endpoint configuration, the target uses the Amazon SDK default credential chain:
...
This allows seamless integration when running on Amazon infrastructure without hardcoding credentials.
...
- Verify Amazon credentials are valid and not expired
...
- Check OpenSearch cluster health in Amazon Console
