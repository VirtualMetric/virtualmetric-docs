...
|Field|Required|Default|Description|
|---|---|---|---|
|`key`|N*|-|Amazon access key ID for authentication|
|`secret`|N*|-|Amazon secret access key for authentication|
|`session`|N|-|Optional session token for temporary credentials|
|`region`|Y|-|Amazon region (e.g., `us-east-1`, `eu-west-1`)|
|`endpoint`|N|-|Custom SQS endpoint URL (for testing or local development)|

\* = Conditionally required. Amazon credentials (`key` and `secret`) are required unless using IAM role-based authentication on Amazon infrastructure.
...
Supports static credentials (access key and secret key) with optional session tokens for temporary credentials. When deployed on Amazon infrastructure, can leverage IAM role-based authentication without explicit credentials.
...
SQS supports dead letter queues (DLQ) for handling messages that cannot be processed successfully. Configure DLQ settings in the Amazon Console or via infrastructure as code.

### Encryption

SQS supports server-side encryption using Amazon KMS. Messages are encrypted at rest. All connections to SQS use HTTPS endpoints for encryption in transit.

### Integration with Amazon Services

SQS integrates with other Amazon services:
- Amazon Lambda for serverless message processing
