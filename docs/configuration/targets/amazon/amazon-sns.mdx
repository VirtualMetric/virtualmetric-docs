...
SNS supports server-side encryption using Amazon KMS. Messages are encrypted in transit and at rest. All connections to SNS use HTTPS endpoints.
...
SNS integrates with other Amazon services:
- Amazon SQS for reliable queuing
- Amazon Lambda for serverless processing
...
:::note
Configure SNS topic subscriptions (SQS, Lambda, HTTP, Email, etc.) separately in the Amazon Console or via infrastructure as code.
:::
