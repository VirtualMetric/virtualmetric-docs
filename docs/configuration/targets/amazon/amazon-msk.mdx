...
Creates a target that writes log messages to Amazon Managed Streaming for Apache Kafka (MSK) topics with support for batching, compression, and Amazon authentication methods. The target handles message delivery efficiently with configurable batch limits based on size or event count. Amazon MSK is a fully managed Apache Kafka service on Amazon.
...
:::note
For IAM authentication, use Amazon IAM roles and policies. IAM authentication is configured at the Amazon MSK cluster level and does not require username/password in the target configuration.
:::
...
Amazon MSK is a fully managed Apache Kafka service on Amazon. This target type allows you to connect to MSK clusters using the standard Kafka protocol.
...
- Username/password authentication stored in Amazon Secrets Manager
- Port: 9094
- Supported algorithms: `scram-sha-256`, `scram-sha-512`
- TLS encryption recommended

**IAM Authentication**
- Uses Amazon IAM roles and policies for authentication
- Port: 9098
- Configured at the Amazon level, not in target configuration
- Requires Amazon credentials configured on the DataStream host
...
:::note
For IAM authentication, configure Amazon credentials on the DataStream host using environment variables, IAM role, or Amazon credentials file. The IAM policy must grant appropriate Kafka permissions.
:::
