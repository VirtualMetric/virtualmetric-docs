...
Each location supports the following properties:

|Field|Required|Default|Description|
|---|---|---|---|
|`id`|Y|-|Unique identifier for the location|
|`path`|Y|-|File output directory|
|`name`|N|From global config|File name template|
|`format`|N|From global config|File format|
|`compression`|N|From global config|Compression algorithm|
|`extension`|N|From global config|Custom file extension|
|`schema`|N|From global config|Data schema or template name|
