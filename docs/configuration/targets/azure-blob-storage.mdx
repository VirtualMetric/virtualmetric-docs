...
|`tenant_id`|Y||Azure tenant ID. Optional when managed identity is enabled|
|`client_id`|Y||Azure client ID. Optional when managed identity is enabled|
|`client_secret`|Y||Azure client secret. Optional when managed identity is enabled|
