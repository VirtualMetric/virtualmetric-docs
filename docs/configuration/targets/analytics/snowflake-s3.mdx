...
| `region` | string | Y | Amazon region for S3 bucket |
| `key` | string | N | Amazon access key ID (uses default credentials chain if omitted) |
| `secret` | string | N | Amazon secret access key |
| `session` | string | N | Amazon session token for temporary credentials |
...
<ExampleGrid>
  <CommentCol>
    Using explicit Amazon credentials for S3 staging access...
  </CommentCol>
  <CodeCol>
    ```yaml
    targets:
...
<ExampleGrid>
  <CommentCol>
    Production-ready configuration with performance tuning, Amazon credentials, and multi-table routing...
  </CommentCol>
  <CodeCol>
    ```yaml
    targets:
