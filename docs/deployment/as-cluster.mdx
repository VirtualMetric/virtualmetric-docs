...
Create a `cluster.yml` file that defines your cluster environment, and place it in the `<vm_root>/Director/config` directory. The example below shows a two-node cluster configuration in the "NewYork" environment:
