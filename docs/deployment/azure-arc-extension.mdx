...
2. **Generate onboarding script** from Azure Portal or CLI:
   ```bash
   az connectedmachine generate-installation-script --resource-group myResourceGroup --location eastus
   ```

3. **Run the script** on each target server:
   ```bash
   # Example for Linux
   bash ./install_arc_agent.sh
   ```
...
3. **Save the template** as `datastream-extension.json`
...
2. **Check extension logs** on the server:
   ...
   # Windows
   type C:\ProgramData\AzureConnectedMachineAgent\Log\DataStreamExtension\*.log
   ```
