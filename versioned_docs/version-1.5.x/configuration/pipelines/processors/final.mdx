...
All these processors control pipeline flow: `final` stops all pipeline processing, <Topic id="processors-return">`return`</Topic> exits the current block, and <Topic id="processors-continue">`continue`</Topic> skips to the next iteration. Use `final` for graceful pipeline termination with clear semantic intent.
:::
